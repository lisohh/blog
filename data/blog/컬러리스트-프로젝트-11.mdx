...
오늘은 React table을 사용해보려고 한다.

## 1-2. react table - column resizing

[리액트 테이블 - 컬럼 리사이징](https://react-table-v7.tanstack.com/docs/examples/column-resizing)

사이트에서 볼 수 있듯이 `헤더`를 움직여서 컬럼의 가로 폭을 조절할 수 있는 기능이다.

## 1-3. react table 설치

터미널에 아래처럼 React-table을 설치하고, 맞는 타입도 설치해준다.
`bun add react-table`
...
![boxcount 배열](/static/images/boxcount 배열.png)
...
색들을 `map`함수를 이용해 인덱스와 컬러 코드를 짝맞추어 배열로 만들어주고,
...
여기서 `role`대신 `{...getTableProps()}`를 넣어주었다.
...
이런 식으로 라이브러리에서 사용한 `Props`들을
나의 경우에 맞추어 변형하고,
변형한 객체들을 알맞게 넣어주려고 노력했다.

아직 `get~~~Props`의 역할을 정확히 이해하지는 못한 것 같아서
몇번 더 읽고 분석해보면서 이해해야겠다.

그리고 라이브러리를 사용하니까 이미 정해둔 폭 이외에 정렬이나 최대 폭 크기를 마음대로
정하는 데 어려운 부분이 있어서, 이부분은 후에 팔레트를 pure css로 구현해서 다시 `react-table`을 입혀보면서
